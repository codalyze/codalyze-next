...
export const Pagination = props => {
  ...
  return (
    <Styled.paginationUl>
      <Styled.paginationLi onClick={onClickFirst}>{"<<"}</Styled.paginationLi>
      {paginationItems.map((item, index) => {
        return (
          <Styled.paginationLi key={index}>{index + 1}</Styled.paginationLi>
        );
      })}
      <Styled.paginationLi onClick={onClickLast}>{">>"}</Styled.paginationLi>
    </Styled.paginationUl>
  );
};
      ...
      <Styled.pageWrapper>
        <Styled.rightImage>
          <OpportunityRight
            width="100%"
            height="100%"
            viewBox="0 0 436.789 448.256"
          />
        </Styled.rightImage>
        <Styled.HomeBannerWrap>
          <HomeBanner title="Find opportunities" />
        </Styled.HomeBannerWrap>

        <Styled.openingsRow>
        <Styled.contentBgTablet></Styled.contentBgTablet>
          <Styled.CareerBg>
            <OpportunityBg
              width="100%"
              height="100%"
